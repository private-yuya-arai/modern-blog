...
tags:\r
  ...
  - HARKing\r
excerpt: >-\r
  （最終回）「統計で嘘をついてはいけない」。p値を恣意的に操作するp-hackingや、結果を見てから仮説を作るHARKingなど、現代科学が直面する再現性の危機と倫理について考えます。\r
image: /images/ethics.png\r
---\r
> [!NOTE]\r
> **AI Update (2026/2/24):** チェリーピッキングを防ぎ分析の客観性を高めるための「詳細な応用知見（事前登録、外れ値処理、感度分析）」を文脈に合わせて追記しました。\r
\r
\r
\r
## この知識はいつ使うの？\r
...
| <span class="ai-body-highlight">**チェリーピッキング**</span> | <span class="ai-body-highlight">都合の良いデータだけを選ぶ。</span> | <span class="ai-body-highlight">論外の捏造行為だが、無意識に「外れ値除去」としてやってしまいがち。</span> |\r
\r
<div class="ai-update-highlight">\r
\r
**💡 2026/2/24 詳細な応用知見:**\r
\r
チェリーピッキング（都合の良いデータの選別）を防ぎ、分析の客観性を担保するためには、以下の3つの実戦的なアプローチが重要です。\r
\r
1. **分析計画の事前登録 (Preregistration)**\r
   データを見る前に、解析対象とする変数、外れ値の定義、除外基準（Exclusion criteria）を確定させます。事後に基準を変更することを防ぎます。\r
\r
2. **客観的な外れ値処理**\r
   主観ではなく、統計的な指標に基づいて処理を行います。\r
   * **Zスコア**: $|z| > 3$（$z = \\frac{x - \\bar{x}}{s}$）を外れ値とする。\r
   * **IQR法**: $[Q_1 - 1.5 \\times IQR, Q_3 + 1.5 \\times IQR]$ の範囲外を除外する。\r
\r
3. **感度分析 (Sensitivity Analysis)**\r
   特定のデータを除外した場合の推定値 $\\hat{\\theta}_{filtered}$ と、全データを用いた推定値 $\\hat{\\theta}_{all}$ の両方を算出し、結論が特定のデータの有無に依存していないか（頑健性）を確認・報告します。\r
\r
---\r
\r
**🛠 チェリーピッキングを防ぐための実戦策:**\r
* **プロトコルの作成**: 分析開始前に「何を成功指標とするか」「どのデータを除外するか」を文書化。\r
* **全結果の報告**: 有意な結果だけでなく、有意にならなかった解析結果もすべて公開する。\r
* **第三者レビュー**: 除外ロジックに恣意性がないか、チーム内で相互確認を行う。\r
\r
</div>\r
\r
\r
